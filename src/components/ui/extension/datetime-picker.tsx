...
const timePickerInputBase =
  "p-1 inline tabular-nums h-fit border-none outline-none select-none content-box caret-transparent rounded-sm min-w-8  text-center focus:bg-foreground/20 focus:dark:bg-foreground/20 focus-visible:ring-0 focus-visible:outline-none";
    ...
    return (
      <div
        className={cn(
          ...
          className,
          "border selection:text-foreground gap-1 rounded-md selection:bg-transparent"
        )}
          ...
          format.map((group, i) => (
            <React.Fragment key={i === 0 ? "dates" : "times"}>
              {group?.length > 0 &&
                group.map((unit, j) => (
                  <React.Fragment key={unit}>
                    <Input
                      className={cn(timePickerInputBase, "w-8", {
                        "min-w-12": unit === "years",
                        "bg-foreground/15 dark:bg-foreground/15":
                          unit === "am/pm",
                      })}
              ...
              {format[1]?.length && !i ? (
                ...
                <span
                  className={cn(timePickerSeparatorBase, "text-xl opacity-30")}
                >
                  |
                </span>
              ) : null}
