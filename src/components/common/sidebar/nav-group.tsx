...
import { NavCollapsible, NavItem, NavLink, type NavGroup } from '@/@types/sidebar';
...
export function NavGroup({ title, items }: Readonly<NavGroup>) {
  ...
  return (
    <SidebarGroup className="px-2 py-0">
      {open && <SidebarGroupLabel>{title}</SidebarGroupLabel>}
      <SidebarMenu>
        {items.map((item) => {
...
const SidebarMenuCollapsible = ({ item, href }: { item: NavCollapsible; href: string }) => {
  ...
  return (
    <Collapsible
      asChild
      defaultOpen={item.isActive ?? checkIsActive(href, item, true)}
